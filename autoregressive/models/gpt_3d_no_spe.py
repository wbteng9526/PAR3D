...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None: # training or naive inference
            ...
            interleaved_ar_tokens = interleave_tokens(
                cam_tokens[:, :self.ar_token_num], 
                token_embeddings[:, :self.ar_token_num]
            ) # b x (2*seq_len) x dim

            interleave_parallel_tokens = interleave_tokens_per_token(
                cam_tokens[:, self.ar_token_num:],
                token_embeddings[:, self.ar_token_num:],
                num_tokens=self.ar_token_num
            )

            token_embeddings = torch.cat((cond_embeddings, interleaved_ar_tokens, interleave_parallel_tokens), dim=1)
        ...
        if self.training:
            ...
            valid_indices = [start_idx + i * 2 for i in range(self.ar_token_num)]
            last_idx = start_idx + self.ar_token_num * 2
            num_groups = (doubled_seq_len-last_idx) // self.ar_token_num
            for i in range(num_groups):
                if i % 2 == 0:
                    valid_indices += [last_idx + j for j in range(i * self.ar_token_num, (i+1) * self.ar_token_num)]
...
def interleave_tokens_per_token(seq1, seq2, num_tokens):
    ...
    result = []
    for i in range(seq1.shape[1] // num_tokens):
        result.append(seq1[:, i*num_tokens:(i+1)*num_tokens])
        result.append(seq2[:, i*num_tokens:(i+1)*num_tokens])
