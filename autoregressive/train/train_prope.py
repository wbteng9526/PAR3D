...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = args.results_dir #f"{args.results_dir}/{experiment_index:03d}-{model_string_name}"  # Create an experiment folder
        ...
        cloud_results_dir = args.cloud_save_path #f"{args.cloud_save_path}/{time_record}"
        cloud_checkpoint_dir = cloud_results_dir #f"{cloud_results_dir}/{experiment_index:03d}-{model_string_name}/checkpoints"
    ...
    if not args.no_wandb and rank == 0:
        ...
        wandb.init(
            ...
            name = f"{cloud_checkpoint_dir}",#f"{timestamp}-{model_string_name}",
            config=vars(args)
        )
    ...
    lr_scheduler = get_scheduler(
        ...
        num_training_steps=args.epochs * len(loader) * dist.get_world_size(),
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, viewmats, Ks, clip_input in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f},  LR {lr_scheduler.get_last_lr()[0]:.5f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--temporal-size", type=int, default=4, help="temporal size of the input")
    parser.add_argument("--downsample-size", type=int, choices=[8, 16], default=16)
    parser.add_argument("--epochs", type=int, default=300)
    parser.add_argument("--lr", type=float, default=5e-4)
    parser.add_argument("--weight-decay", type=float, default=5e-3, help="Weight decay to use")
    ...
    # lr scheduler
    parser.add_argument("--lr-scheduler-type", type=str, default='cosine', choices=['cosine', 'cosine_with_restarts', 'polynomial', 'constant', 'constant_with_warmup', 'piecewise_constant'])
    parser.add_argument("--num-warmup-steps", type=int, default=10000)
    parser.add_argument("--min-lr-ratio", type=float, default=0.05)
    parser.add_argument("--num-cycles", type=float, default=0.5)
    # rvq
